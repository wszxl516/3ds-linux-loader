...
void show_info(const char *format, ...)
{
    if (format && *format) { // only if there is something in there
        ...
        str_width = GetDrawStringWidth(str);
        str_height = GetDrawStringHeight(str);
        x = (str_width >= SCREEN_WIDTH_TOP) ? 0 : (0);
        y = (str_height >= SCREEN_HEIGHT) ? 0 : (str_height + (str_height / 2 * current_line));
        DrawStringF(TOP_SCREEN, x, y, COLOR_STD_FONT, COLOR_STD_BG, str);
        current_line += 1;
    } else ClearScreenF(true, false, COLOR_STD_BG);
}
...
void show_warn(const char *format, ...)
{
    if (format && *format) { // only if there is something in there
        ...
        x = (str_width >= SCREEN_WIDTH_TOP) ? 0 : (0);
        y = (str_height >= SCREEN_HEIGHT) ? 0 : (str_height + (str_height / 2 * current_line));
        DrawStringF(TOP_SCREEN, x, y, COLOR_ORANGE, COLOR_STD_BG, str);
        current_line += 1;
    } else ClearScreenF(true, false, COLOR_STD_BG);
}

bool warn_prompt(bool ask, const char *format, ...)
{
    u32 str_width, str_height;
    u32 x, y;
    bool ret = true;
    ...
    x = (str_width >= SCREEN_WIDTH_TOP) ? 0 : (0);
    y = (str_height >= SCREEN_HEIGHT) ? 0 : (str_height + (str_height / 2 * current_line));
    DrawStringF(TOP_SCREEN, x, y, COLOR_BLUE, COLOR_STD_BG, str);
    DrawStringF(TOP_SCREEN, x, y + str_height - (1*10), COLOR_GREEN, COLOR_STD_BG, (ask) ? "(<A> yes, <B> no)" : "(<A> to continue)");
    current_line += 1;
    while (true) {
    ...
    if (ShowInputPrompt(inputstr, 128 + 1, 2, alphabet, format, va)) {
        *size = strnlen(inputstr, 128) / 2;
        for (u32 i = 0; i < *size; i++) {
            char bytestr[2 + 1] = { 0 };
            unsigned int byte;
            bytestr[0] = *(inputstr + (2*i));
            bytestr[1] = *(inputstr + (2*i) + 1);
...
bool ShowProgress(u64 current, u64 total, const char* opstr)
{
    ...
    snprintf(tempstr, 128, "%s (%lu%%)", progstr, prog_percent);
...
char* readable_size(float size, char *buf, u32 buf_size) {
    int i = 0;
